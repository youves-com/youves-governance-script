...
const voteHash = 'ipfs://QmRLojnxo3HCEwgCf5oJEUDjKkj5WMmZiXirichrL4YUFm'
const blockLevel = 1943070
...
const run = async () => {
  ...
  const getYOUHoldings = async (
    ...
    return {
      ...
      youInUusdPool: youInUusdPool.integerValue().toString()
    }
  }
  ...
  getVotes().then(async (votes) => {
    ...
    writeFileSync(
      './votes.csv',
      `Address,Vote,TotalYOU,youInWallet,youInStakingPool,youInQuipuswapPool,youInPlentyPool,youInUusdPool\n` +
        holdings
          .map((row) =>
            [
              row.address,
